using Belmondo.Mathematics;
using Raylib_cs.BleedingEdge;
using System.Numerics;
using System.Text;
using static Belmondo.FightFightDanger.Math2;
...
public class RaylibRenderer
{
    ...
    public void Initialize(in World world)
    {
        ...
        GameWorldRenderTexture = LoadRenderTexture(VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT);
        OuterRenderTexture = LoadRenderTexture(VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT);
        SetTextureFilter(OuterRenderTexture.Texture, TextureFilter.Trilinear);
    }

    public void Shutdown()
    {
        UnloadRenderTexture(GameWorldRenderTexture);
        UnloadRenderTexture(OuterRenderTexture);
        RaylibResources.UnloadAll();
        CloseAudioDevice();
        CloseWindow();
    }

    public void Render(in Game game, in TimeContext timeContext, UIState uiState)
    {
        BeginTextureMode(GameWorldRenderTexture);
        {
        ...
        BeginTextureMode(OuterRenderTexture);
        {
            BeginShaderMode(RaylibResources.DownmixedShader);
            {
                DrawTexture(GameWorldRenderTexture.Texture, 0, 0, Color.White);
            }
            EndShaderMode();
        }
        EndTextureMode();
    ...
    private void DrawMenu(in Game game)
    {
    ...
    private void RenderWandering(in World world, in TimeContext timeContext)
    {
    ...
    private void RenderBattle(in Battle battle, in TimeContext timeContext)
    {
        ...
        BattleCamera.Position.X = x;
        BattleCamera.Target = BattleCamera.Position + Vector3.UnitZ;

        BeginMode3D(BattleCamera);
        {
            Rlgl.DisableBackfaceCulling();
